...
void ASurfaceRendererContext::setWindowBuffer(JNIEnv* env, int64_t contentId, AHardwareBuffer* ahb, int fenceFd, int64_t windowId, int64_t serial, jobject ahbImage, int slot) {
    ...
    // CPU buffers (ahbImage != null) are already converted to RGBA in drawable.c when ASR mode is enabled.
    // GPU buffers pass through directly - any format conversion should be handled by the GPU renderer.
